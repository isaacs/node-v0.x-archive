...
function url_format (obj) {
  ...
  var out = [];
  if (obj.hasOwnProperty("protocol") && obj.protocol !== undefined) {
    out.push(obj.protocol);
    if (obj.protocol.substr(-1) !== ":") out.push(":");
  }
  
  if ( obj.hasOwnProperty("host") && obj.host !== undefined) {
    out.push("//", obj.host);
  } else if (obj.hasOwnProperty("hostname") && obj.hostname !== undefined) {
    out.push("//");
    if (obj.hasOwnProperty("auth") && obj.auth !== undefined) out.push(obj.auth, "@");
    out.push(obj.hostname);
    if (obj.hasOwnProperty("port") && obj.port !== undefined) out.push(":", obj.port);
  }
  
  if (obj.hasOwnProperty("pathname") && obj.pathname !== undefined) {
    if ((obj.host || obj.hostname) && obj.pathname.charAt(0) !== "/") out.push("/");
    out.push(obj.pathname);
  }
  if (obj.hasOwnProperty("search") && obj.search !== undefined) {
    if (obj.search.charAt(0) !== "?") out.push("?");
    out.push(obj.search);
  }
  if (obj.hasOwnProperty("hash") && obj.hash !== undefined) {
    if (obj.hash.charAt(0) !== "#") out.push("#");
    out.push(obj.hash);
  }
  return out.join("");
};
