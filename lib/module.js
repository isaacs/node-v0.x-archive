...
function firstCompile (module, data, cb) {
  ...
  // at this point, if the data returned is "string", and the
  // module is not already ".js" compile as JavaScript.
  if (cb) {
...
function resolve (root, id, parent, cb) {
  ...
  if (parent && searchPaths.length === 1) {
    ...
    if (shortCircuit in parent.moduleCache) {
      if (cb) {
        cb(null, parent.moduleCache[shortCircuit]);
      }
      return parent.moduleCache[shortCircuit];
    }
  }
