...
// WARNING: DONT EVER USE THIS FUNCTION IN PRODUCTION WEBSITES!!
...
function cat (message) {
  ...
  //   if (part.filename) stream.files[part.filename] = part.body;
  //   if (part.name) stream.fields[part.name] = part.body;
  // });
...
// A non-multipart message looks just like a multipart message with a single part.
function Stream (message) {
  var isMultiPart = multipartHeaders(message, this);
  message.addListener("body", isMultiPart ? writer(this) : simpleWriter(this));
    ...
    while (buffer.length > 0) {
      ...
      switch (state) {
        case S_NEW_PART:
          ...
          // either way, we expect --boundary right away.
