...
Interface.prototype._searchNext = function () {
  this._buildSearch();
  if (this.search.index > 0) {
    this.search.index --;
    this.line = this.search[this.search.index];
    this._refreshLine();
  }
}
Interface.prototype._searchPrev = function () {
  this._buildSearch();
  if (this.search.index + 1 < this.search.length) {
    this.search.index++;
    this.line = this.search[this.search.index];
    this._refreshLine();
  }
}
Interface.prototype._buildSearch = function () {
  // if the search hasn't changed, then don't touch it.
  if (this.search && this.search.length &&
      this.search.cursor === this.cursor &&
      this.search.query === this.line.substr(0, this.cursor)) return;
  var s = this.line.substr(0, this.cursor);
  var uniq = [];
  this.search = this.history.filter(function (h) {
    var ok = h.indexOf(s) === 0 && uniq.indexOf(h) === -1;
    uniq.push(h);
    return ok;
  });
  this.search.query = s;
...
Interface.prototype._historyNext = function () {
  if (this.cursor !== 0
      && !(this.cursor === this.line.length && this.historyIndex > -1)
      && process.env.NODE_READLINE_SEARCH) {
...
Interface.prototype._historyPrev = function () {
  if (this.cursor !== 0
      && !(this.cursor === this.line.length && this.historyIndex >= 0)
      && process.env.NODE_READLINE_SEARCH) {
    return this._searchPrev();
  }
  ...
  switch (b[0]) {
    ...
    case 27:    /* escape sequence */
      ...
      if (b[1] === 98 && this.cursor > 0) {
        ...
        var del = this.line.substr(this.cursor);
        this.line = this.line.substr(0, this.cursor);
        del = del.replace(/^\s*/, '').split(/\b(?=[\w]|$)/).slice(1).join("");
        this.line += del;
        this._refreshLine();
        break;
      ...
      } else if (b[1] === 127 && this.cursor > 0) {
        // meta-backspace, delete word
        var del = this.line.substr(0, this.cursor);
        this.line = this.line.substr(del.length);
        del = del.replace(/\s*$/, '').split(/\b(?=[\w]|$)/).slice(0, -1).join("");
        this.line = del + this.line;
        this.cursor = del.length;
        this._refreshLine();
      } else if (b[1] === 27 && b[2] === 91 && b[3] === 67) {
        // meta-arrow-right, word-right
        // the regexp is: a word boundary, followed by a word char or the eol
        this.cursor += this.line.substr(this.cursor).split(/\b(?=[\w]|$)/)[0].length
        this.cursor = Math.min(this.line.length, this.cursor);
        this._refreshLine();
      } else if (b[1] === 27 && b[2] === 91 && b[3] === 68) {
        // meta-left-arrow, word-left
        // the regexp is: a word boundary, followed by a word char or the eol
        this.cursor -= this.line.substr(0, this.cursor).split(/\b(?=[\w]|$)/).pop().length
