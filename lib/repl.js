...
REPLServer.prototype.complete = function(line) {
  ...
  if (match) {
    ...
    if (!subdir) {
      ...
      var builtinLibs = ['assert', 'child_process', 'crypto', 'dgram',
        'dns', 'events', 'file', 'fs', 'http', 'net', 'os', 'path',
        'querystring', 'readline', 'repl', 'util', 'tcp',
        'url'];
...
function defineDefaultCommands(repl) {
  ...
  repl.defineCommand('n', {
    help: 'Load a native module (* for all)',
    action: function(mod) {
      if (arguments.length === 1 && mod.trim().match(/\s/)) {
        return arguments.callee.apply(this, mod.trim().split(/\s+/));
      }
      if (arguments.length === 1 && !mod.trim()) {
        arguments[0] = "*";
      }
      for (var i = 0, l = arguments.length; i < l; i ++) {
        ...
        var loaded = {};
        if (a === '*') {
          this.outputStream.write('Loading all native modules\n');
          return arguments.callee.apply(this, Object.keys(
            process.binding('natives')));
        }
        if (process.binding('natives')[a]) {
          this.outputStream.write('Loading '+a+'\n');
          loaded[a] = this.context[a] = require(a);
        } else {
          this.outputStream.write('Not a native module: '+a+'\n');
        }
      }
      this.displayPrompt();
    }
